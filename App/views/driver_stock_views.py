...
# Blueprint for driver-specific stock routes, prefixed with /api/driver
...
def get_driver_or_403(user_id):
    ...
    # This assumes current_user.id is the ID of the User model, 
    # which is the primary key for the Driver model via inheritance.
    driver = Driver.query.get(user_id)
    # Check if the user exists and is indeed a Driver (although role_required should handle the latter)
    if not driver: 
        return None
    return driver
...
@jwt_required()
# Assuming role_required('Driver') is applied elsewhere or in a security wrapper
def view_driver_stock_endpoint():
    ...
    # Note: If you use role_required('Driver') here, the check above can be simplified.
    if not driver:
        # Returning 403 as the user is authenticated but not authorized as a Driver
        return jsonify({"error": "Unauthorized or user is not a driver"}), 403

    try:
        # Call the controller function to get the DriverStock objects
        stocks = driver_controller.driver_view_stock(driver)
        
        # Format the response data to include item names
        stock_data = []
        for stock in stocks:
            # Ensure the item relationship is loaded before accessing .name
        ...
        # Catch unexpected errors like database connection issues
...
def update_driver_stock_endpoint():
    ...
    try:
        # Call the controller function to update the stock
        updated_stock = driver_controller.driver_update_stock(driver, item_id, quantity)
        
        # Format and return the updated stock data
        stock_data = {
            'id': updated_stock.id,
        ...
        # Handle business logic errors from the controller (e.g., "Invalid item ID.")
